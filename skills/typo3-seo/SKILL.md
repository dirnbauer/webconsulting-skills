...
> **TYPO3 v14:** Per Breaking #104422, sitemap GET parameters are namespaced as `tx_seo[sitemap]` and `tx_seo[page]`. The `PageTypeSuffix` enhancer above handles only the page type number. Core deliberately leaves sitemap pagination as query parameters. If you want clean child sitemap URLs, add a **custom** route enhancer (not Core-shipped):
...
> This enhancer is **optional and custom** — Core's `route-enhancers.yaml` only maps `tx_seo/sitemap` and leaves pagination as query params by design. Test thoroughly after adding.
