...
### PHPStan Level 9 Patterns for TYPO3 Extensions

Level 9 enforces strict `mixed` handling. TYPO3 APIs often return `mixed` from settings,
repositories, and ViewHelper arguments. These patterns satisfy level 9 without baselines.
...
{
    /**
     * @return QueryResultInterface<int, Page>
     */
...
#### Narrowing `mixed` from TYPO3 APIs

```php
// ❌ PHPStan level 9 error: Cannot call method on mixed
$value = $this->renderingContext->getVariableProvider()->get('data');
$title = $value['title'];
...
#### assert() for Null-Narrowing

```php
// When you know a value can't be null at this point
...
#### instanceof Over @var Casts

```php
// ❌ PHPStan level 9 rejects @var that narrows beyond the declared type
/** @var AbstractTemplateView $view */
$view = $this->view;

// ✅ Use instanceof for safe type narrowing
