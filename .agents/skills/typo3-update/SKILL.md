...
## 3. Dual-Version Compatibility Gotchas

These patterns address real API differences between v13 and v14 that are easy to miss.

### #[Validate] Attribute

TYPO3 13's `ValidateAttribute` constructor does not accept named parameters. For dual compatibility, use the array form:

```php
// ❌ Works only on TYPO3 14 (named parameter)
...
### ViewInterface in Controllers

In v14, `$this->view` returns `ViewInterface` (not `AbstractTemplateView`). If you need template-view-specific methods, guard with `instanceof`:
...
In v14, `$this->renderingContext` can be `null`. Always add a null guard before accessing it:
...
public function render(): string
{
    ...
    $provider = $this->renderingContext->getVariableProvider();
    // ...
}
```

### UserFunction / FlexForm DI

Classes called via `itemsProcFunc` in FlexForm may bypass the DI container. Make DI optional:
...
### PHPStan TYPO3 Extension

For dual v13/v14 PHPStan support, widen the constraint:

```json
"saschaegerer/phpstan-typo3": "^2.0 || ^3.0"
```

## 4. Controller Patterns (v13/v14 Compatible)
...
## 5. View & Templating (v13/v14 Compatible)
...
## 6. Event System (v13/v14 Compatible)
...
## 7. Backend Module Registration (v13/v14)
...
## 8. TCA Configuration (v13/v14 Compatible)
...
## 9. Database Operations (v13/v14 Compatible)
...
## 10. CLI Commands (v13/v14 Compatible)
...
## 11. Testing for Dual-Version Compatibility
