...
## Real-World Pitfalls (v13/v14 Dual Support)

Lessons learned from actual extension upgrades to dual v13/v14 support.

### composer.lock Must Not Be Committed for Extensions

Extensions should **not** ship a `composer.lock`. It pins to a single TYPO3 version and breaks CI matrix testing. Remove it:
...
In CI, always use `composer update` (not `composer install`) so Composer resolves dependencies fresh for each matrix combination:
...
Use `composer validate --strict --no-check-lock` since there's no lock file.

### SPDX License Identifiers

`composer validate --strict` rejects deprecated license identifiers. Use SPDX 2.0+ format:
...
### PHPStan TYPO3 Extension Version Constraints

`saschaegerer/phpstan-typo3` v2 supports TYPO3 13, v3 supports TYPO3 14. For dual support:
...
### #[Validate] Attribute Differences

TYPO3 13's `ValidateAttribute` constructor does **not** accept named parameters. TYPO3 14's does. For dual compatibility, always use the array form:

```php
// ❌ Fails on TYPO3 13 (named parameter not accepted)
...
### ViewInterface vs AbstractTemplateView

In TYPO3 14, `$this->view` in `ActionController` returns `ViewInterface`, not `AbstractTemplateView`. If you need view-specific methods (e.g., `getViewTemplatePaths()`), use `instanceof`:

```php
// ❌ Fails PHPStan on v14 (ViewInterface doesn't have these methods)
/** @var AbstractTemplateView $view */
$view = $this->view;

// ✅ Safe for both versions
if ($this->view instanceof AbstractTemplateView) {
    $paths = $this->view->getRenderingContext()
        ->getTemplatePaths();
}
```

### UserFunction / ItemsProcFunc DI

Classes instantiated by TYPO3's FlexForm processing (e.g., `itemsProcFunc`) may **not** go through the DI container. Make constructor DI optional with a `GeneralUtility` fallback:
...
> **Note:** `final readonly` classes cannot have nullable constructor parameters with manual assignment. Drop `readonly` if you need this pattern.

### ViewHelper renderingContext Nullability

In TYPO3 14, `$this->renderingContext` in ViewHelpers can be `null`. Always add a null guard:
...
### Functional Tests in DDEV Need DB Grants

The TYPO3 Testing Framework creates temporary databases (e.g., `db_ft408cdc5`). The default `db` user lacks permission to create them. Grant access first:

```bash
ddev mysql -uroot -proot -e "GRANT ALL ON \`db_%\`.* TO 'db'@'%'; FLUSH PRIVILEGES;"
```

Then run with environment variables:

```bash
typo3DatabaseName=db typo3DatabaseHost=db typo3DatabaseUsername=db \
typo3DatabasePassword=db typo3DatabaseDriver=mysqli \
ddev exec vendor/bin/phpunit -c Tests/FunctionalTests.xml
```
