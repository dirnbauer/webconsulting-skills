...
### Multi-Version Matrix (Recommended)

For extensions supporting both TYPO3 13 and 14, use a matrix strategy with `composer update`
(not `composer install`) so dependencies resolve fresh for each TYPO3 version.

> **Critical:** Do NOT commit `composer.lock` for extensions. It pins to a single TYPO3
> version and breaks matrix testing. Add it to `.gitignore`.
    ...
    strategy:
      ...
      matrix:
        include:
          ...
          - php-version: '8.4'
            typo3-version: '^14.0'
    name: "Unit (PHP ${{ matrix.php-version }}, TYPO3 ${{ matrix.typo3-version }})"
    steps:
      ...
      - name: Run unit tests
        run: vendor/bin/phpunit -c Tests/UnitTests.xml
    ...
    strategy:
      ...
      matrix:
        include:
          ...
          - php-version: '8.3'
            typo3-version: '^14.0'
    name: "Functional (PHP ${{ matrix.php-version }}, TYPO3 ${{ matrix.typo3-version }})"
    services:
      mysql:
        image: mysql:8.0
        env:
...
### Running Functional Tests in DDEV

The TYPO3 Testing Framework creates temporary databases. The default `db` user needs permission:

```bash
# Grant permission to create temp test databases
ddev mysql -uroot -proot -e "GRANT ALL ON \`db_%\`.* TO 'db'@'%'; FLUSH PRIVILEGES;"

# Run with DDEV database credentials
