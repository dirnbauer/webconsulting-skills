...
Two proven subtitle styles are available. Choose based on content density.

#### Style A: Word-by-word reveal (verbose narration)

- **Auto-sync**: Component reads `NARRATION_SCRIPT[scene]` directly
- **Slide-up entrance**: Subtitles slide up 20px while fading in (0.4s–0.9s)
- **Glass morphism container**: Dark gradient background with subtle border
- **Word-by-word reveal**: Each word fades from 0.25 to 1.0 opacity at ~2.8 words/second
- **Active word glow**: Currently spoken word gets full opacity + soft white `text-shadow`
- **Typography**: SF Pro Display / Inter, weight 300 (light), 30px

#### Style B: Short pill chunks (recommended for technical content)

Splits narration into chunks of ~5 words. Displays one chunk at a time in a pill-shaped
blurred container. Feels cinematic and prevents subtitle text from competing with
on-screen content like code examples or diagrams.

```tsx
function NarrationSubtitle({ text, sceneDurationFrames }: {
  text: string;
  ...
  const framesPerChunk = sceneDurationFrames / totalChunks;

  const currentChunkIndex = Math.min(
    Math.floor(frame / framesPerChunk),
    totalChunks - 1
  );
  const chunkLocalFrame = frame - currentChunkIndex * framesPerChunk;

  const fadeIn = interpolate(chunkLocalFrame, [0, fps * 0.3], [0, 1], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
  const fadeOut = interpolate(
    chunkLocalFrame, [framesPerChunk - fps * 0.3, framesPerChunk], [1, 0],
    { extrapolateLeft: "clamp", extrapolateRight: "clamp" }
  );
  const slideY = interpolate(chunkLocalFrame, [0, fps * 0.3], [8, 0], {
    extrapolateLeft: "clamp", extrapolateRight: "clamp",
  });
...
- Style A: Product tours with dense information and few visual elements
- Style B: Technical videos with code, diagrams, or screenshots where subtitles should not distract
...
ElevenLabs may mispronounce product names, acronyms, or technical terms. Add a
`applyPronunciationFixes()` function to the narration script that replaces problematic
text before sending to the API:

```typescript
function applyPronunciationFixes(text: string): string {
  return text
    .replace(/TYPO3/g, "TYPO three")     // Prevents "typOi3" or "type-oh-3"
    .replace(/DDEV/g, "D-Dev")           // Prevents "d-d-e-v"
    .replace(/PHPStan/g, "PHP Stan")     // Prevents "fip-stan"
    .replace(/npm/g, "N P M");           // Spells out acronym
}
```

**Testing pronunciations:** Create a test script that generates multiple variants of
the same word with different phonetic spellings and compare the audio output:
...
// Generate one MP3 per variant, listen, pick the best
```
